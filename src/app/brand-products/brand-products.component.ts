...
export class BrandProductsComponent implements OnInit,OnDestroy {
  ...
  GetProducts(){
    this.loader = true;
    const data = {token:localStorage.getItem('u'),page:this.page,slug:this.catID,sub_cat_id:this.sub_cat_id,product_name:$('#product_name').val(),price_array:this.price_array,rating:this.rating,discount:this.discount,offer:this.offer};
    this.appService.login('get/brand-products',data).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
